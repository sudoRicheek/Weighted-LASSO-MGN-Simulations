...
addpath('Helper Functions', '../Pooling Matrix Design/Balanced Binary Matrices/Generation');

load('../Infection Spread Model/Data/ct_data_1.mat');
...
for ss=1:nsig
    ...
    % Uncomment the next line if you wish to optimize the pooling matrix
    % with respect to phi (incorporate side information in encoder)
%     B = phi_optimize_balanced(B, logical(CCT));
...
fprintf('\nCOMP NNLASSO: [%.5f,%.5f] & [%.5f,%.5f] & [%.5f,%.5f] & [%.5f,%.5f] & [%.5f,%.5f]',...
